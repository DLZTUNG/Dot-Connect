...
public class GridManager : MonoBehaviour
{
    ...
    private void SetGridState()
    {
        ...
        for (int x = 0; x < m_gridSizeX; x++)
        {
            for (int y = 0; y < m_gridSizeY; y++)
            {
                m_gridState[x, y] = 0; // default là chưa có màu nào
            }
        }
    }
    ...
    private void CreateGrid()
    {
        ...
        for (int x = 0; x < m_gridSizeX; x++)
        {
            for (int y = 0; y < m_gridSizeY; y++)
            {
                ...
                cellScript.cellState = Enum.CellState.None;
    ...
    private Dot CreateDot(Vector2Int gridPos, int colorId)
    {
        ...
        {
            if (cell.gridPos == gridPos && cell.cellState == Enum.CellState.None)
            {
                cell.cellState = Enum.CellState.Dot;
                break;
            }
        }
        return dotComp;
    ...
    public Dot GetDotByPosInConnection(Vector2Int pos)
    {
        foreach (var dot in m_dots)
        {
    ...
    public bool IsHoveredValid(Vector2Int pos)
    {
        return pos != m_unValidVector;
    }
